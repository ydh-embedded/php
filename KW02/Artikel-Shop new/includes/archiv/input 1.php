...
class input implements ArrayAccess, Countable, Iterator
{
    ...
    function _slug($data)
    {
        return preg_replace("/[^\w-]+|^[^a-z]+|[^\w]+$|(?<=[-_])[-_]+/", "", strtolower($data));
    }
    ...
    function _xss($data)
    {
        if (preg_match("/[<&>]/", $data)) {   // looks remotely like html
            ...
            // alert patterns
            if (
                preg_match("#[<<]/*(\?import|applet|embed|object|script|style|!\[CDATA\[|title|body|link|meta|base|i?frame|frameset|i?layer)#iUu", $html, $uu)
